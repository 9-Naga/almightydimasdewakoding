...
public class DisbursementController {
    ...
    public DisbursementController(DisbursementService disbursementService, LoanApplicationService loanApplicationService, AuthService authService) {
    ...
    @GetMapping("/pending")
    @PreAuthorize("@permissionEvaluator.hasPermission(authentication, 'LOAN_DISBURSE') or hasRole('BACK_OFFICE')")
    @Operation(summary = "Get pending disbursements", description = "BACK_OFFICE - Get approved loans pending disbursement")
    ...
    @PostMapping("/{loanId}")
    @PreAuthorize("@permissionEvaluator.hasPermission(authentication, 'LOAN_DISBURSE') or hasRole('BACK_OFFICE')")
    @Operation(summary = "Process disbursement", description = "BACK_OFFICE - Process loan disbursement")
    public ResponseEntity<ApiResponse<DisbursementResponse>> processDisbursement(@PathVariable Long loanId) {
        CustomUserDetails currentUser = authService.getCurrentUser();
        DisbursementResponse disbursement = disbursementService.processDisbursement(loanId, currentUser.getId());
        
        ApiResponse<DisbursementResponse> response = ApiResponse.<DisbursementResponse>builder()
                .success(true)
    ...
    public ResponseEntity<ApiResponse<List<DisbursementResponse>>> getAllDisbursements() {
        ...
        ApiResponse<List<DisbursementResponse>> response = ApiResponse.<List<DisbursementResponse>>builder()
                .success(true)
    ...
    @Operation(summary = "Get disbursement by loan ID", description = "Get disbursement details for a specific loan")
    public ResponseEntity<ApiResponse<DisbursementResponse>> getDisbursementByLoanId(@PathVariable Long loanId) {
