...
public class ApprovalController {
  ...
  @GetMapping("/pending")
  @PreAuthorize(
      "@permissionEvaluator.hasPermission(authentication, 'LOAN_APPROVE') or"
          + " hasRole('BRANCH_MANAGER')")
  @Operation(
      summary = "Get pending approvals",
      description = "BRANCH_MANAGER - Get reviewed loans pending approval")
  ...
  @Operation(
      ...
      description = "BRANCH_MANAGER - Approve or reject loan application")
  public ResponseEntity<ApiResponse<LoanApprovalResponse>> submitApproval(
      @PathVariable Long loanId, @Valid @RequestBody LoanApprovalRequest request) {
    CustomUserDetails currentUser = authService.getCurrentUser();
    LoanApprovalResponse approval =
        loanApprovalService.submitApproval(loanId, currentUser.getId(), request);

    ApiResponse<LoanApprovalResponse> response =
        ApiResponse.<LoanApprovalResponse>builder()
            .success(true)
  ...
  @Operation(
      ...
      description = "BRANCH_MANAGER - Get approvals by current user")
  public ResponseEntity<ApiResponse<List<LoanApprovalResponse>>> getMyApprovals() {
    CustomUserDetails currentUser = authService.getCurrentUser();
    List<LoanApprovalResponse> approvals =
        loanApprovalService.getApprovalsByApprover(currentUser.getId());

    ApiResponse<List<LoanApprovalResponse>> response =
        ApiResponse.<List<LoanApprovalResponse>>builder()
            .success(true)
  ...
  @Operation(
      summary = "Get approval by loan ID",
      description = "Get approval details for a specific loan")
  public ResponseEntity<ApiResponse<LoanApprovalResponse>> getApprovalByLoanId(
