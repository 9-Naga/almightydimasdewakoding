...
public class ReviewController {
    ...
    public ReviewController(LoanReviewService loanReviewService, LoanApplicationService loanApplicationService, AuthService authService) {
    ...
    @GetMapping("/pending")
    @PreAuthorize("@permissionEvaluator.hasPermission(authentication, 'LOAN_REVIEW') or hasRole('MARKETING')")
    @Operation(summary = "Get pending reviews", description = "MARKETING - Get loan applications pending review")
    ...
    @PostMapping("/{loanId}")
    @PreAuthorize("@permissionEvaluator.hasPermission(authentication, 'LOAN_REVIEW') or hasRole('MARKETING')")
    @Operation(summary = "Submit review", description = "MARKETING - Submit review for a loan application")
    public ResponseEntity<ApiResponse<LoanReviewResponse>> submitReview(
        ...
        LoanReviewResponse review = loanReviewService.submitReview(loanId, currentUser.getId(), request);
        
        ApiResponse<LoanReviewResponse> response = ApiResponse.<LoanReviewResponse>builder()
                .success(true)
    ...
    @PreAuthorize("@permissionEvaluator.hasPermission(authentication, 'LOAN_REVIEW') or hasRole('MARKETING')")
    @Operation(summary = "Get my reviews", description = "MARKETING - Get reviews submitted by current user")
    public ResponseEntity<ApiResponse<List<LoanReviewResponse>>> getMyReviews() {
        ...
        ApiResponse<List<LoanReviewResponse>> response = ApiResponse.<List<LoanReviewResponse>>builder()
                .success(true)
    ...
    @Operation(summary = "Get review by loan ID", description = "Get review details for a specific loan")
    public ResponseEntity<ApiResponse<LoanReviewResponse>> getReviewByLoanId(@PathVariable Long loanId) {
