...
public class LoanApprovalService {
  ...
  @Transactional
  public LoanApprovalResponse submitApproval(
      Long loanId, Long approverId, LoanApprovalRequest request) {
    ...
    if (loan.getStatus() != LoanStatus.IN_REVIEW) {
      throw new BadRequestException(
          "Loan application must be reviewed by marketing first (status: IN_REVIEW)");
    }
  ...
  public LoanApprovalResponse getApprovalByLoanId(Long loanId) {
    LoanApproval approval =
        loanApprovalRepository
            ...
            .orElseThrow(
                () -> new ResourceNotFoundException("Approval not found for loan: " + loanId));
